...
//            表明这个 Socket 在设置的端口上监听数据。
            client = new DatagramSocket(Constants.LINSEN_SOCKET_UDP_PORT);
...
//            每次接收完UDP数据后，重置长度。否则可能会导致下次收到数据包被截断。
    ...
    **/
     ...
     * 发送信息
     **/
    public void sendMessageBytes(final byte[] message) {
        if (client == null) {
            startUDPSocket();
        }
        mThreadPool.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    DatagramPacket packet = new DatagramPacket(message, message.length, targetAddress, Constants.SOCKET_UDP_PORT);
                    client.send(packet);
                } catch (IOException e) {
                    e.printStackTrace();
                }
